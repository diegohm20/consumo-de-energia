...
if not hasattr(st.session_state, "logged_in") or not st.session_state.logged_in:
    ...
    if uploaded_file is not None:
        try:
            ...
            st.error(
                "Erro ao ler o arquivo CSV. Verifique se o formato está correto.")
        except KeyError as e:
            st.error(
                f"A coluna '{e}' não foi encontrada no arquivo CSV. Certifique-se de que as colunas 'sigla_uf', 'consumo', 'numero_consumidores' e 'tipo_consumo' estão presentes.")

    else:
        st.info(
            "Por favor, carregue um arquivo CSV com os dados de consumo de energia.")
