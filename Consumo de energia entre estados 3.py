    ...
    try:
        ...
        if not all(col in df.columns for col in ["sigla_uf", "consumo", "numero_consumidores", "tipo_consumo"]):
            ...
            fig_bar_estado = px.bar(consumo_por_estado_ordenado, x="sigla_uf", y="consumo_total", labels={
                                    "sigla_uf": "Estado", "consumo_total": "Consumo Total de Energia"}, title="Comparação do Consumo Total de Energia por Estado")
            ...
            fig_bar_medio_estado = px.bar(consumo_medio_por_estado_ordenado, x="sigla_uf", y="consumo_medio", labels={
                                          "sigla_uf": "Estado", "consumo_medio": "Consumo Médio por Consumidor"}, title="Consumo Médio de Energia por Consumidor por Estado")
            st.plotly_chart(fig_bar_medio_estado)
            st.dataframe(consumo_medio_por_estado_ordenado)
            ...
            fig_bar_medio_tipo = px.bar(consumo_medio_por_tipo_ordenado, x="tipo_consumo", y="consumo_medio", labels={
                                        "tipo_consumo": "Tipo de Consumo", "consumo_medio": "Consumo Médio por Consumidor"}, title="Consumo Médio de Energia por Consumidor por Tipo de Consumo")
            st.plotly_chart(fig_bar_medio_tipo)
            st.dataframe(consumo_medio_por_tipo_ordenado)
            ...
            st.dataframe(consumo_medio_estado_tipo)
